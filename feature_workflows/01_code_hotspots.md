...
| `src/trotters_trader/dashboard.py` | about `2030` lines, about `71` defs | routing, HTML rendering, formatting, operator view-model shaping, auth / CSRF behavior, and now promotion-path plus active-branch, runbook-queue, and next-family panels are tightly coupled | `FW-002`, `FW-008`, `FW-010`, `FW-012`, `FW-014`, `FW-015`, `FW-016`, `FW-018`, `FW-021`, `FW-022` |
...
| `src/trotters_trader/runbook_queue.py` | runbook parsing, portfolio alignment, active-plan drift warnings, queue-level operator messaging, and now approval-state blockers live in one summary builder | this contract is now the only compact explanation of whether the supervisor queue is runnable, blocked pending approval, or bootstrap-ready | `FW-015`, `FW-016`, `FW-019`, `FW-021` |
| `src/trotters_trader/research_families.py` plus `configs/research_family_proposals/*.json` | proposal validation, approval-state comparison, next-family status, and bootstrap orchestration now form one governance surface | this is now the approval boundary between research ideas and supervisor queue execution, so status drift or weak validation would reopen unsafe continuation paths | `FW-017`, `FW-018`, `FW-019`, `FW-020`, `FW-021` |
