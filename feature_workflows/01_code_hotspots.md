...
| `src/trotters_trader/api.py` plus `src/trotters_trader/ops_bridge.py` | auth, audit, mutation policy, transport concerns, and now promotion-path read surfaces are hand-rolled WSGI flows | security posture and operator contracts can drift if request validation and read-model shaping diverge | `FW-008`, `FW-009`, `FW-011`, `FW-012` |
