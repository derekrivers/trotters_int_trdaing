...
| --- | --- | --- | --- |
| `src/trotters_trader/research_runtime.py` | about `3629` lines, about `103` defs | runtime state, campaign orchestration, director orchestration, notifications, agent dispatch, service-heartbeat exports, launch-claim recovery, and now backend-aware DB orchestration all live together | `FW-003`, `FW-005`, `FW-006`, `FW-008`, `FW-014`, `FW-029`, `FW-030` |
| `src/trotters_trader/dashboard.py` | about `2030` lines, about `71` defs | routing, HTML rendering, formatting, operator view-model shaping, auth / CSRF behavior, and now promotion-path plus active-branch, runbook-queue, next-family, and backlog-continuity panels are tightly coupled | `FW-002`, `FW-008`, `FW-010`, `FW-012`, `FW-014`, `FW-015`, `FW-016`, `FW-018`, `FW-021`, `FW-022`, `FW-023`, `FW-026`, `FW-027`, `FW-028`, `FW-029` |
...
| `src/trotters_trader/api.py` plus `src/trotters_trader/ops_bridge.py` | auth, audit, mutation policy, transport concerns, promotion-path read surfaces, overview compaction, active-branch visibility, queue governance, and now family-comparison / backlog-continuity summaries all live in hand-rolled WSGI flows | security posture and operator contracts can drift if request validation, payload size, and read-model shaping diverge | `FW-008`, `FW-009`, `FW-011`, `FW-012`, `FW-013`, `FW-014`, `FW-015`, `FW-016`, `FW-018`, `FW-021`, `FW-022`, `FW-023`, `FW-026` |
| `src/trotters_trader/runtime_overview.py` | shared full/compact overview shaping, notifications, terminal summaries, and runtime-health logic now sit behind both the dashboard and API surfaces | cross-surface drift will return quickly if new overview fields bypass this shared layer or presentation code starts rebuilding operator read models ad hoc | `FW-026` |
| `src/trotters_trader/runtime_db.py` | runtime DB connection policy, SQL placeholder translation, backend detection, and retryable DB error semantics are now centralized here | this seam is the boundary that keeps Postgres rollout work from re-fragmenting SQL and connection logic back into `research_runtime.py` | `FW-030` |
...
| `src/trotters_trader/eodhd.py` plus `src/trotters_trader/staging.py` plus `src/trotters_trader/canonical.py` | vendor API entitlement assumptions, symbol normalization, LSE cash-unit scaling, and total-return reconstruction now interact across download, staging, and canonical layers | data-quality regressions here can silently change adjusted-close semantics or break the managed EODHD ingestion path even when backtests still execute | `FW-024` |
| `configs/eodhd_momentum_total_return.toml` plus `configs/research_family_proposals/momentum_total_return_starter_guard.json` plus `configs/research_programs/momentum_total_return_starter_guard.json` | the managed total-return data path now participates directly in governed queue selection instead of existing as a side-path experiment | if these artifacts drift, operators can believe the queue is testing data-contract improvements when it is actually still evaluating the old vendor-adjusted path | `FW-025` |
...
| `docker-compose.yml` plus local env posture | host exposure, healthchecks, restart policy, worker scale baseline, and now staged runtime-DB rollout controls all live in one deployment file | security, rollout, and fault-tolerance regressions can hide in config drift even when the Python code is correct | `FW-008`, `FW-030` |
